...
internal class SeriesUpdaterService : ISeriesUpdaterService
{
        ...
        ScanlationGroupName groupName)
    {
        ...
        if (seriesExternalId != savedSeries.ExternalId
            || relativeCoverUrl != savedSeries.RelativeCoverUrl)
        {
            updatedSeries = new Series
            {
                Id = savedSeries.Id,
                ScanlationGroupId = savedSeries.ScanlationGroupId,
                ExternalId = seriesExternalId,
                Title = savedSeries.Title,
                RelativeCoverUrl = relativeCoverUrl,
            };

            seriesRepository.UpdateSeries(updatedSeries);

            if (updatedSeries.ExternalId != savedSeries.ExternalId)
            {
                _logger.LogInformation(
                    "Updated external Id of series with Id {SeriesId} to {SeriesExternalId}",
                    updatedSeries.Id,
                    updatedSeries.ExternalId);
            }

            if (updatedSeries.RelativeCoverUrl != savedSeries.RelativeCoverUrl)
            {
                _logger.LogInformation(
                    "Updated cover URL for series with Id {SeriesId} and " +
                    "external Id {SeriesExternalId}",
                    updatedSeries.Id,
                    updatedSeries.ExternalId);
            }
        }
        ...
        var chaptersAdded = await AddChaptersAsync(
            seriesRepository,
            scrapedSeries,
            urlManager,
            updatedSeries ?? savedSeries,
            updateDate,
            groupName,
            latestSavedChapter);

        return updatedSeries is not null || chaptersAdded;
    }
